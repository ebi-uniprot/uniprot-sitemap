...
module.exports = ({ namespace, query = "*" } = {}) => {
  // Note: can't seem to be able to customise fields without losing modifiedDate
  const baseURL = `https://rest.uniprot.org/${namespace}/search?format=json&query=${query}`;
    ...
    while (nextURL || data) {
      ...
      for (const result of data?.results || []) {
        if (result) {
          ...
          yield {
            // called "uniRuleId", even for ARBA 🤷🏼
            accession: result.uniRuleId,
